...
//record the number of command the replica deal with for each key
var KeyInfo = make([]ppgenericsmrproto.InstanceInfo, ppgenericsmrproto.KEY_NUM)
var KeyAllocate = make([]int32, ppgenericsmrproto.KEY_NUM)

type Replica struct {
	*ppgenericsmr.Replica
	...
	repliProposeChan      chan fastrpc.Serializable //modified by Luna in 2018.12.20
	repliProposeReplyChan chan fastrpc.Serializable //modified by Luna in 2018.12.20
	prepareRPC            uint8
	...
	repliProposeRPC       uint8 //modified by Luna in 2018.12.20
	repliProposeReplyRPC  uint8 //modified by Luna in 2018.12.20
...
type LeaderBookkeeping struct {
	clientProposals   []*ppgenericsmr.Propose
	maxRecvBallot     int32
	prepareOKs        int
	allEqual          bool
	preAcceptOKs      int
...
func NewReplica(id int, peerAddrList []string, portnum int, thrifty bool, exec bool, dreply bool, beacon bool, durable bool) *Replica {
	r := &Replica{
		ppgenericsmr.NewReplica(id, peerAddrList, portnum, thrifty, exec, dreply),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE*3),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE*3),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE*2),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE*3),
		make(chan fastrpc.Serializable, ppgenericsmr.CHAN_BUFFER_SIZE*3),
		...
		-1,
		new(sync.Mutex),
		make(chan *instanceId, ppgenericsmr.CHAN_BUFFER_SIZE)}
	...
	//register RPCs
	r.prepareRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.Prepare), r.prepareChan)
	r.prepareReplyRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.PrepareReply), r.prepareReplyChan)
	r.preAcceptRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.PreAccept), r.preAcceptChan)
	r.preAcceptReplyRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.PreAcceptReply), r.preAcceptReplyChan)
	r.preAcceptOKRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.PreAcceptOK), r.preAcceptOKChan)
	r.acceptRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.Accept), r.acceptChan)
	r.acceptReplyRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.AcceptReply), r.acceptReplyChan)
	r.commitRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.Commit), r.commitChan)
	r.commitShortRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.CommitShort), r.commitShortChan)
	r.tryPreAcceptRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.TryPreAccept), r.tryPreAcceptChan)
	r.tryPreAcceptReplyRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.TryPreAcceptReply), r.tryPreAcceptReplyChan)
	//modified by Luna in 2018.12.20
	r.repliProposeRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.RepliPropose), r.repliProposeChan)
	r.repliProposeReplyRPC = ppgenericsmr.RegisterRPC(r.Replica, new(ppepaxosproto.RepliProposeReply), r.repliProposeReplyChan)
...
/* ============= */
//modified by Luna in 2018.12.20
//passed the information of keys
func (r *Replica) EDealInfo(args *ppgenericsmrproto.EDealInfoArgs, reply *ppgenericsmrproto.EDealInfoReply) error {
	for i := 0; i < ppgenericsmrproto.KEY_NUM; i++ {
		reply.KeyInfo[i].Slow = KeyInfo[i].Slow
		reply.KeyInfo[i].Total = KeyInfo[i].Total
		KeyInfo[i].Slow = 0
		KeyInfo[i].Total = 0
	}
	return nil
}
//update the KeyAllocate
func (r *Replica) UpdateKeyAllocate(args *ppgenericsmrproto.UpdateKeyAllocateArgs, reply *ppgenericsmrproto.UpdateKeyAllocateReply) error {
	for i := 0; i < ppgenericsmrproto.KEY_NUM; i++ {
		KeyAllocate[i] = r.Id
	}
	for i := 0; i < args.KeyNo; i++ {
		KeyAllocate[args.KeyToReplica[i].Key] = args.KeyToReplica[i].Replica
	}
	return nil
}
//pass the propose command
func (r *Replica) passPropose(propose *ppgenericsmr.Propose) {
	args := &ppepaxosproto.RepliPropose{propose.CommandId, propose.Command, propose.Timestamp, propose.Reply, r.Id}
	ppgenericsmr.SendMsg(r.Replica, KeyAllocate[propose.Command.K], r.repliProposeRPC, args)
}
//var replipro ppgenericsmr.Propose // for case repropose := <-r.repliProposeChan use
/* ============= */
...
func (r *Replica) run() {
	ppgenericsmr.ConnectToPeers(r.Replica)

	dlog.Println("Waiting for client connections")

	go ppgenericsmr.WaitForClientConnections(r.Replica)
	...
	if r.Id == 0 {
		...
		ppgenericsmr.UpdatePreferredPeerOrder(r.Replica, quorum)
	}
	...
	for !r.Shutdown {
		...
		case propose := <-onOffProposeChan:
			...
			if KeyAllocate[propose.Command.K] != r.Id {
				r.passPropose(propose) //to pass the propose
			} else {
		...
		case reproposeS := <-r.repliProposeChan :
			repropose := reproposeS.(*ppepaxosproto.RepliPropose)
			replipro := &ppgenericsmr.Propose{&ppgenericsmrproto.Propose{repropose.CommandId, repropose.Command, repropose.Timestamp}, repropose.Index, repropose.RepliNo}
			dlog.Printf("Proposal with op %d\n", replipro.Command.Op)
			r.handlePropose(replipro)
			break

		case prepareS := <-r.prepareChan:
			prepare := prepareS.(*ppepaxosproto.Prepare)
		...
		case preAcceptS := <-r.preAcceptChan:
			preAccept := preAcceptS.(*ppepaxosproto.PreAccept)
		...
		case acceptS := <-r.acceptChan:
			accept := acceptS.(*ppepaxosproto.Accept)
		...
		case commitS := <-r.commitChan:
			commit := commitS.(*ppepaxosproto.Commit)
		...
		case commitS := <-r.commitShortChan:
			commit := commitS.(*ppepaxosproto.CommitShort)
			//got a Commit message
			dlog.Printf("Received Commit for instance %d.%d\n", commit.LeaderId, commit.Instance)
		...
		case prepareReplyS := <-r.prepareReplyChan:
			prepareReply := prepareReplyS.(*ppepaxosproto.PrepareReply)
			//got a Prepare reply
		...
		case preAcceptReplyS := <-r.preAcceptReplyChan:
			preAcceptReply := preAcceptReplyS.(*ppepaxosproto.PreAcceptReply)
		...
		case preAcceptOKS := <-r.preAcceptOKChan:
			preAcceptOK := preAcceptOKS.(*ppepaxosproto.PreAcceptOK)
			//got a PreAccept reply
		...
		case acceptReplyS := <-r.acceptReplyChan:
			acceptReply := acceptReplyS.(*ppepaxosproto.AcceptReply)
			//got an Accept reply
		...
		case tryPreAcceptS := <-r.tryPreAcceptChan:
			tryPreAccept := tryPreAcceptS.(*ppepaxosproto.TryPreAccept)
		...
		case tryPreAcceptReplyS := <-r.tryPreAcceptReplyChan:
			tryPreAcceptReply := tryPreAcceptReplyS.(*ppepaxosproto.TryPreAcceptReply)
		...
		//modified by Luna in 2018.12.20
		case repliProposeReS := <-r.repliProposeReplyChan:
			repliProposeRe := repliProposeReS.(*ppepaxosproto.RepliProposeReply)
			ppgenericsmr.ReplyProposeTS(
                                    r.Replica,
				&ppgenericsmrproto.ProposeReplyTS{
					repliProposeRe.OK,
					repliProposeRe.CommandId,
					repliProposeRe.Value,
					repliProposeRe.Timestamp},
				r.ClientConn[repliProposeRe.Index])
			KeyInfo[repliProposeRe.K].Slow++
			KeyInfo[repliProposeRe.K].Total++
			break

		case beacon := <-r.BeaconChan:
			dlog.Printf("Received Beacon from replica %d with timestamp %d\n", beacon.Rid, beacon.Timestamp)
			ppgenericsmr.ReplyBeacon(r.Replica, beacon)
			break
		...
		case <-slowClockChan:
			if r.Beacon {
				for q := int32(0); q < int32(r.N); q++ {
					...
					ppgenericsmr.SendBeacon(r.Replica, q)
				}
			}
			break
	...
	for !r.Shutdown {
		...
		for q := 0; q < r.N; q++ {
			...
			for inst = r.ExecedUpTo[q] + 1; inst < r.crtInstance[q]; inst++ {
				if r.InstanceSpace[q][inst] != nil && r.InstanceSpace[q][inst].Status == ppepaxosproto.EXECUTED {
					if inst == r.ExecedUpTo[q]+1 {
						r.ExecedUpTo[q] = inst
					}
					continue
				}
				if r.InstanceSpace[q][inst] == nil || r.InstanceSpace[q][inst].Status != ppepaxosproto.COMMITTED {
...
func (r *Replica) replyPrepare(replicaId int32, reply *ppepaxosproto.PrepareReply) {
	ppgenericsmr.SendMsg(r.Replica, replicaId, r.prepareReplyRPC, reply)
}

func (r *Replica) replyPreAccept(replicaId int32, reply *ppepaxosproto.PreAcceptReply) {
	ppgenericsmr.SendMsg(r.Replica, replicaId, r.preAcceptReplyRPC, reply)
}

func (r *Replica) replyAccept(replicaId int32, reply *ppepaxosproto.AcceptReply) {
	ppgenericsmr.SendMsg(r.Replica, replicaId, r.acceptReplyRPC, reply)
}

func (r *Replica) replyTryPreAccept(replicaId int32, reply *ppepaxosproto.TryPreAcceptReply) {
	ppgenericsmr.SendMsg(r.Replica, replicaId, r.tryPreAcceptReplyRPC, reply)
}
...
func (r *Replica) bcastPrepare(replica int32, instance int32, ballot int32) {
	...
	args := &ppepaxosproto.Prepare{r.Id, replica, instance, ballot}
	...
	for sent := 0; sent < n; {
		...
		ppgenericsmr.SendMsg(r.Replica, q, r.prepareRPC, args)
		sent++
	}
}

var pa ppepaxosproto.PreAccept
...
func (r *Replica) bcastPreAccept(replica int32, instance int32, ballot int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		ppgenericsmr.SendMsg(r.Replica, r.PreferredPeerOrder[q], r.preAcceptRPC, args)
		sent++
		if sent >= n {
			break
		}
...
var tpa ppepaxosproto.TryPreAccept
...
func (r *Replica) bcastTryPreAccept(replica int32, instance int32, ballot int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	for q := int32(0); q < int32(r.N); q++ {
		...
		ppgenericsmr.SendMsg(r.Replica, q, r.tryPreAcceptRPC, args)
	}
}

var ea ppepaxosproto.Accept
...
func (r *Replica) bcastAccept(replica int32, instance int32, ballot int32, count int32, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		ppgenericsmr.SendMsg(r.Replica, r.PreferredPeerOrder[q], r.acceptRPC, args)
		sent++
		if sent >= n {
			break
		}
...
var ec ppepaxosproto.Commit
var ecs ppepaxosproto.CommitShort
...
func (r *Replica) bcastCommit(replica int32, instance int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		if r.Thrifty && sent >= r.N/2 {
			ppgenericsmr.SendMsg(r.Replica, r.PreferredPeerOrder[q], r.commitRPC, args)
		} else {
			ppgenericsmr.SendMsg(r.Replica, r.PreferredPeerOrder[q], r.commitShortRPC, argsShort)
			sent++
		}
	}
}
...
func (r *Replica) updateCommitted(replica int32) {
	for r.InstanceSpace[replica][r.CommittedUpTo[replica]+1] != nil &&
		(r.InstanceSpace[replica][r.CommittedUpTo[replica]+1].Status == ppepaxosproto.COMMITTED ||
			r.InstanceSpace[replica][r.CommittedUpTo[replica]+1].Status == ppepaxosproto.EXECUTED) {
...
func (r *Replica) handlePropose(propose *ppgenericsmr.Propose) {
	...
	proposals := make([]*ppgenericsmr.Propose, batchSize)
...
func (r *Replica) startPhase1(replica int32, instance int32, ballot int32, proposals []*ppgenericsmr.Propose, cmds []state.Command, batchSize int) {
...
func (r *Replica) handlePreAccept(preAccept *ppepaxosproto.PreAccept) {
	...
	if inst != nil && (inst.Status == ppepaxosproto.COMMITTED || inst.Status == ppepaxosproto.ACCEPTED) {
	...
	status := ppepaxosproto.PREACCEPTED_EQ
	if changed {
		status = ppepaxosproto.PREACCEPTED
	}

	if inst != nil {
		if preAccept.Ballot < inst.ballot {
			r.replyPreAccept(preAccept.LeaderId,
				&ppepaxosproto.PreAcceptReply{
					preAccept.Replica,
					preAccept.Instance,
					FALSE,
					inst.ballot,
	...
	if changed || uncommittedDeps || preAccept.Replica != preAccept.LeaderId || !isInitialBallot(preAccept.Ballot) {
		...
		pok := &ppepaxosproto.PreAcceptOK{preAccept.Instance}
		ppgenericsmr.SendMsg(r.Replica, preAccept.LeaderId, r.preAcceptOKRPC, pok)
	}

	dlog.Printf("I've replied to the PreAccept\n")
}

func (r *Replica) handlePreAcceptReply(pareply *ppepaxosproto.PreAcceptReply) {
	dlog.Printf("Handling PreAccept reply\n")
	inst := r.InstanceSpace[pareply.Replica][pareply.Instance]

	if inst.Status != ppepaxosproto.PREACCEPTED {
	...
	if inst.lb.preAcceptOKs >= r.N/2 && inst.lb.allEqual && isInitialBallot(inst.ballot) {//modified by Luna in 2018.12.20
		happy++
		dlog.Printf("Fast path for instance %d.%d\n", pareply.Replica, pareply.Instance)
		r.InstanceSpace[pareply.Replica][pareply.Instance].Status = ppepaxosproto.COMMITTED
		...
		if inst.lb.clientProposals != nil && !r.Dreply {
			...
			for i := 0; i < len(inst.lb.clientProposals); i++ {
				...
				if inst.lb.clientProposals[i].IsRepli == -1 {
					ppgenericsmr.ReplyProposeTS(
                                        	r.Replica,
						&ppgenericsmrproto.ProposeReplyTS{
							TRUE,
					...
					KeyInfo[inst.Cmds[i].K].Total++
				} else { // the command is from a replica
					args := &ppepaxosproto.RepliProposeReply{TRUE, inst.lb.clientProposals[i].CommandId, inst.Cmds[i].K, state.NIL, inst.lb.clientProposals[i].Timestamp, inst.lb.clientProposals[i].Reply}
					ppgenericsmr.SendMsg(r.Replica, inst.lb.clientProposals[i].IsRepli, r.repliProposeReplyRPC, args)
				}
			}
		} else if inst.lb.clientProposals != nil {//modified by Luna in 2018.12.20
			for i := 0; i < len(inst.lb.clientProposals); i++ {
				if inst.lb.clientProposals[i].IsRepli == -1 {
					KeyInfo[inst.Cmds[i].K].Total++
				}
		...
		inst.Status = ppepaxosproto.ACCEPTED
...
func (r *Replica) handlePreAcceptOK(pareply *ppepaxosproto.PreAcceptOK) {
	dlog.Printf("Handling PreAccept reply\n")
	...
	if inst.lb.preAcceptOKs >= r.N/2 && inst.lb.allEqual && isInitialBallot(inst.ballot) {//modified by Luna in 2018.12.20
		happy++
		r.InstanceSpace[r.Id][pareply.Instance].Status = ppepaxosproto.COMMITTED
...
func (r *Replica) handleAccept(accept *ppepaxosproto.Accept) {
	...
	if inst != nil && (inst.Status == ppepaxosproto.COMMITTED || inst.Status == ppepaxosproto.EXECUTED) {
		return
	}
	...
	if inst != nil {
		if accept.Ballot < inst.ballot {
			r.replyAccept(accept.LeaderId, &ppepaxosproto.AcceptReply{accept.Replica, accept.Instance, FALSE, inst.ballot})
			return
		}
		inst.Status = ppepaxosproto.ACCEPTED
		inst.Seq = accept.Seq
...
func (r *Replica) handleAcceptReply(areply *ppepaxosproto.AcceptReply) {
	inst := r.InstanceSpace[areply.Replica][areply.Instance]

	if inst.Status != ppepaxosproto.ACCEPTED {
	...
	if inst.lb.acceptOKs+1 > r.N/2 {
		r.InstanceSpace[areply.Replica][areply.Instance].Status = ppepaxosproto.COMMITTED
...
func (r *Replica) handleCommit(commit *ppepaxosproto.Commit) {
...
func (r *Replica) handleCommitShort(commit *ppepaxosproto.CommitShort) {
	inst := r.InstanceSpace[commit.Replica][commit.Instance]
...
func (r *Replica) startRecoveryForInstance(replica int32, instance int32) {
	...
	if r.InstanceSpace[replica][instance] == nil {
		r.InstanceSpace[replica][instance] = &Instance{nil, 0, ppepaxosproto.NONE, 0, nildeps, nil, 0, 0, nil}
	}
	...
	} else if inst.Status >= ppepaxosproto.PREACCEPTED {
...
func (r *Replica) handlePrepare(prepare *ppepaxosproto.Prepare) {
	inst := r.InstanceSpace[prepare.Replica][prepare.Instance]
	var preply *ppepaxosproto.PrepareReply
	var nildeps [DS]int32
	...
	if inst == nil {
		r.InstanceSpace[prepare.Replica][prepare.Instance] = &Instance{
			...
			nildeps,
			nil, 0, 0, nil}
		preply = &ppepaxosproto.PrepareReply{
			r.Id,
			prepare.Replica,
			prepare.Instance,
			TRUE,
...
func (r *Replica) handlePrepareReply(preply *ppepaxosproto.PrepareReply) {
	...
	if preply.Status == ppepaxosproto.COMMITTED || preply.Status == ppepaxosproto.EXECUTED {
	...
	if preply.Status == ppepaxosproto.ACCEPTED {
	...
	if (preply.Status == ppepaxosproto.PREACCEPTED || preply.Status == ppepaxosproto.PREACCEPTED_EQ) &&
		(inst.lb.recoveryInst == nil || inst.lb.recoveryInst.status < ppepaxosproto.ACCEPTED) {
		...
		} else if preply.Status == ppepaxosproto.PREACCEPTED_EQ {
	...
	if ir != nil {
		...
		if ir.status == ppepaxosproto.ACCEPTED ||
			(!ir.leaderResponded && ir.preAcceptCount >= r.N/2 && (r.Thrifty || ir.status == ppepaxosproto.PREACCEPTED_EQ)) {
			...
			if conf, q, i := r.findPreAcceptConflicts(ir.cmds, preply.Replica, preply.Instance, ir.seq, ir.deps); conf {
				if r.InstanceSpace[q][i].Status >= ppepaxosproto.COMMITTED {
				...
				inst.Seq = ir.seq
				inst.Deps = ir.deps
				inst.Status = ppepaxosproto.PREACCEPTED
...
func (r *Replica) handleTryPreAccept(tpa *ppepaxosproto.TryPreAccept) {
	inst := r.InstanceSpace[tpa.Replica][tpa.Instance]
	if inst != nil && inst.ballot > tpa.Ballot {
		// ballot number too small
		r.replyTryPreAccept(tpa.LeaderId, &ppepaxosproto.TryPreAcceptReply{
			r.Id,
			tpa.Replica,
			tpa.Instance,
			FALSE,
	...
	if conflict, confRep, confInst := r.findPreAcceptConflicts(tpa.Command, tpa.Replica, tpa.Instance, tpa.Seq, tpa.Deps); conflict {
		...
		r.replyTryPreAccept(tpa.LeaderId, &ppepaxosproto.TryPreAcceptReply{r.Id, tpa.Replica, tpa.Instance, TRUE, inst.ballot, 0, 0, 0})
	}
}
	...
	for q := int32(0); q < int32(r.N); q++ {
		for i := r.ExecedUpTo[q]; i < r.crtInstance[q]; i++ {
			...
			if state.ConflictBatch(inst.Cmds, cmds) {
				if i > deps[q] ||
					(i < deps[q] && inst.Seq >= seq && (q != replica || inst.Status > ppepaxosproto.PREACCEPTED_EQ)) {
					// this is a conflict
					return true, q, i
				}
			}
...
func (r *Replica) handleTryPreAcceptReply(tpar *ppepaxosproto.TryPreAcceptReply) {
	...
	if tpar.OK == TRUE {
		...
		if tpar.ConflictStatus >= ppepaxosproto.COMMITTED || notInQuorum > r.N/2 {
