...
func initReplica() *Replica {
	peers := make([]string, 3)
	r := &Replica{ppgenericsmr.NewReplica(0, peers, false, false, false),
		make(chan *Propose, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.Prepare, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.PreAccept, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.PreAccept, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.Accept, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.Commit, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.PrepareReply, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.PreAcceptReply, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.PreAcceptOK, CHAN_BUFFER_SIZE),
		make(chan *ppepaxosproto.AcceptReply, CHAN_BUFFER_SIZE),
		make([][]*Instance, 3),
		make([]int32, 3),
		false,
		0,
...
func (r *Replica) MakeInstance(q, i int, seq int32, deps [3]int32) {
	...
	r.InstanceSpace[q][i] = &Instance{command, 0, ppepaxosproto.COMMITTED, seq, deps, nil, 0, 0}
}
