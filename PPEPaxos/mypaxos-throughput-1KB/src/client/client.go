...
func main() {
	...
	f, err := os.Create("./epaxos.txt")//record the time in .txt
	if err != nil{
		fmt.Println(err)
	}
	...
	//modified by Luna in 2018.12.30
	var argsValue state.Value
	for i := 0; i < 127; i++ {
		argsValue[i] = 0
	}
	args := genericsmrproto.Propose{id, state.Command{state.PUT, 0, argsValue}, 0}
	...
	for j := 0; j < *rounds; j++ {
		...
		if *check {
			for k := 0+j*n; k < n+j*n; j++ {
				if !rsp[k] {
					fmt.Println("Didn't receive", k)
				}
			}
		}
