...
func (t *RepliPropose) BinarySize() (nbytes int, sizeKnown bool) {
	return 41, true
}

type RepliProposeCache struct {
	mu    sync.Mutex
	cache []*RepliPropose
}

func NewRepliProposeCache() *RepliProposeCache {
	c := &RepliProposeCache{}
	c.cache = make([]*RepliPropose, 0)
	return c
}

func (p *RepliProposeCache) Get() *RepliPropose {
	var t *RepliPropose
	p.mu.Lock()
	if len(p.cache) > 0 {
		t = p.cache[len(p.cache)-1]
...
func (p *RepliProposeCache) Put(t *RepliPropose) {
	p.mu.Lock()
	p.cache = append(p.cache, t)
	p.mu.Unlock()
}

func (t *RepliPropose) New() fastrpc.Serializable {
	return new(RepliPropose)
}
...
func (t *RepliPropose) Unmarshal(wire io.Reader) error {
	...
	t.Index = int64((uint64(bs[0]) | (uint64(bs[1]) << 8) | (uint64(bs[2]) << 16) | (uint64(bs[3]) << 24) | (uint64(bs[4]) << 32) | (uint64(bs[5]) << 40) | (uint64(bs[6]) << 48) | (uint64(bs[7]) << 56)))
	bs = b[:4]
	if _, err := io.ReadAtLeast(wire, bs, 4); err != nil {
		return err
	}
	t.RepliNo = int32((uint32(bs[0]) | (uint32(bs[1]) << 8) | (uint32(bs[2]) << 16) | (uint32(bs[3]) << 24)))
	return nil
}

func (t *RepliProposeReply) BinarySize() (nbytes int, sizeKnown bool) {
	return 37, true
}

type RepliProposeReplyCache struct {
	mu    sync.Mutex
	cache []*RepliProposeReply
}

func NewRepliProposeReplyCache() *RepliProposeReplyCache {
	c := &RepliProposeReplyCache{}
	c.cache = make([]*RepliProposeReply, 0)
	return c
}

func (p *RepliProposeReplyCache) Get() *RepliProposeReply {
	var t *RepliProposeReply
	p.mu.Lock()
	if len(p.cache) > 0 {
		t = p.cache[len(p.cache)-1]
		p.cache = p.cache[0:(len(p.cache) - 1)]
...
func (p *RepliProposeReplyCache) Put(t *RepliProposeReply) {
	p.mu.Lock()
	p.cache = append(p.cache, t)
	p.mu.Unlock()
}

func (t *RepliProposeReply) New() fastrpc.Serializable {
	return new(RepliProposeReply)
}

func (t *RepliProposeReply) Marshal(wire io.Writer) {
	var b [8]byte
	var bs []byte
	bs = b[:5]
	bs[0] = byte(t.OK)
...
func (t *RepliProposeReply) Unmarshal(wire io.Reader) error {
	var b [8]byte
	var bs []byte
	bs = b[:5]
